...
import static com.egakat.io.commons.solicitudes.domain.salidas.Salida.AUTORIZADO_IDENTIFICACION;
import static com.egakat.io.commons.solicitudes.domain.salidas.Salida.AUTORIZADO_NOMBRES;
import static com.egakat.io.commons.solicitudes.domain.salidas.Salida.PREDISTRIBUCION_CROSSDOCK;
import static com.egakat.io.commons.solicitudes.domain.salidas.Salida.REQUIERE_RECAUDO;
import static com.egakat.io.commons.solicitudes.domain.salidas.Salida.VALOR_RECAUDAR;
...
public class MapEntidadSalidaDecorator extends MapEntidadDecorator<Salida, Long> {

	public MapEntidadSalidaDecorator(Decorator<Salida, Long> inner) {
		super(inner);
	}

	@Override
	protected Salida map(EtlRequestDto<Salida, Long> archivo, RegistroDto<Salida, Long> registro) {
		...
		Integer valorRecaudar = getInteger(archivo, registro, VALOR_RECAUDAR);

		val result = new Salida();
		...
		result.setPredistribucionCrossdock(
				StringUtils.defaultString(getString(archivo, registro, PREDISTRIBUCION_CROSSDOCK)));

		result.setValorUnitarioDeclarado(valorUnitarioDeclarado);

		result.setAutorizadoIdentificacion(
				StringUtils.defaultString(getString(archivo, registro, AUTORIZADO_IDENTIFICACION)));
		result.setAutorizadoNombres(StringUtils.defaultString(getString(archivo, registro, AUTORIZADO_NOMBRES)));
		result.setRequiereRecaudo(StringUtils.defaultString(getString(archivo, registro, REQUIERE_RECAUDO)));
		result.setValorRecaudar(valorRecaudar);
